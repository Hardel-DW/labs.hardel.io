...
export async function PUT(request: NextRequest, { params }: Params) {
    try {
        const userId = await new RestUsers(request).getUser();
        const { id, data }: { id: string; data: CreateCategoryModel } = await request.json();

        const response = await new CategoryRepository(prisma.category).update(params.identifier, userId, id, data);
        return NextResponse.json(response);
    } catch (error: any) {
        return NextResponse.json(new RestErrorHandler(ErrorType.InternalServerError, error), { status: StatusCode.InternalServerError });
    }
